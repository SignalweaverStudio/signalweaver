...
class ExecutionLog(Base):
    """Records the outcome of an execution attempt linked to a DecisionTrace."""
    ...
    decision: Mapped[str] = mapped_column(String(16))  # proceed/gate/refuse
    connector: Mapped[str] = mapped_column(String(64), default="mock")
    status: Mapped[str] = mapped_column(String(16))  # executed / blocked
    response_json: Mapped[str] = mapped_column(Text, default="")


# ============================================================
# Alert Dispatch Log (Stage 21)
# ============================================================

class AlertDispatchLog(Base):
    """Audit record for outbound alert delivery attempts.

    One row per POST /alerts/dispatch call. Captures whether alerts
    were found, whether dispatch was attempted, and the outcome.
    Sensitive fields (signing_secret, authorization, etc.) are
    ...
    # "ok" if no alerts were found, "alert" if alerts were present
    alert_status: Mapped[str] = mapped_column(String(16))

    # Number of triggered alerts
    alert_count: Mapped[int] = mapped_column(Integer, default=0)

    # "not_sent" | "sent" | "failed"
    dispatch_status: Mapped[str] = mapped_column(String(16))

    # Connector name used for dispatch (e.g. "webhook")
    connector: Mapped[str] = mapped_column(String(64), default="webhook")

    # Sanitized JSON result from the dispatch attempt.
    # Redacted before storage — no secrets ever persisted.
