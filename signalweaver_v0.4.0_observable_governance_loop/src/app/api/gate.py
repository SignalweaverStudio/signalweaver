...
from app.gate import UserState, decide, apply_enforcement_mode
...
def _run_gate_evaluation(
    *,
    ...
    """
    Shared gate evaluation pipeline.

    Returns a dict with all evaluation results including:
      decision, reason, conflicted_ids, log_id, trace_id,
      enforcement_mode, would_block, explanation, max_level,
      raw_decision, final_decision, explanations_list
    This function is used by both /gate/evaluate and /execute/trusted
    so evaluation logic is never duplicated.
    """
    # 1) Resolve enforcement mode from profile (or default hard)
    ...
    # 2) Load active anchors scoped to tenant
    stmt_all = (
        select(TruthAnchor)
        .where(TruthAnchor.active == True)  # noqa: E712
        .where(
            (TruthAnchor.tenant_id == tenant_id) | (TruthAnchor.tenant_id == None)  # noqa: E711
        )
    )
    active_anchors = list(db.scalars(stmt_all).all())

    # 3) Run conflict detection
    conflicts, match_debug = _detect_conflicts(request_summary, active_anchors)

    explanations_list = _build_explanations(request_summary, conflicts)
    ...
    # 4) Run core decision logic (enforcement-mode-agnostic)
    raw_decision = decide(
        state=UserState(
            arousal=_norm_state(arousal),
            dominance=_norm_state(dominance),
    ...
    final_decision = apply_enforcement_mode(
        raw_decision, enforcement_mode, max_level_conflict=max_level
    )
    would_block = raw_decision.would_block
    ...
    try:
        ...
        # 8) Snapshot tenant-scoped anchors considered
        anchors_now = list(db.scalars(stmt_all).all())

        for a in anchors_now:
            snap = DecisionTraceAnchor(
                ...
                active_snapshot=a.active,
                matched=(a.id in conflicted_ids),
                match_note="conflict",
            )
            db.add(snap)
    ...
    return {
        ...
        "suggestion": final_decision.suggestion,
...
def evaluate(payload: GateEvaluateIn, db: Session = Depends(get_db), tenant: Tenant = Depends(get_tenant)):
    ...
    return GateEvaluateOut(
        ...
        explanations=ev["explanations_list"],
        next_actions=ev["next_actions"],
        ethos_refs=_ethos_refs_for(ev["decision"], ev["max_level"]),
        warnings=ev["warnings"],
        warning_anchors=[AnchorOut.model_validate(a, from_attributes=True) for a in ev["warning_anchors"]],
...
def reframe(payload: GateReframeIn, db: Session = Depends(get_db), tenant: Tenant = Depends(get_tenant)):
    ...
    # FIX 6: use explicit None check so empty string "" doesn't fall back to parent value
    ...
    # Re-run conflict detection on the reframed text (tenant-scoped)
    stmt_all = (
        select(TruthAnchor)
        .where(TruthAnchor.active == True)  # noqa: E712
        .where(
    ...
    # Create DecisionTrace for the reframed evaluation
    reframed_explanation_text = " | ".join(_build_explanations(reframed, conflicts)) if decision.decision != "proceed" else ""

    trace = DecisionTrace(
        policy_profile_id=None,
        request_text=reframed,
        request_normalized=_norm(reframed),
        arousal=arousal,
        dominance=dominance,
        decision=decision.decision,
        reason=decision.reason,
        explanation=reframed_explanation_text,
        match_debug_json=json.dumps({
            "conflicted_ids": conflicted_ids,
            "max_level_conflict": max_level,
            "matcher": "naive",
            "reframed_from_log_id": parent.id,
        }, ensure_ascii=False),
...
def replay(trace_id: int, db: Session = Depends(get_db), tenant: Tenant = Depends(get_tenant)):
    ...
    # FIX 3: use _detect_conflicts() so SW_MATCHER env var is respected during replay
    ...
    # FIX 4: re-build explanation from current conflicts so same_explanation is meaningful
    ...
    return ReplayOut(
        ...
        # FIX 4: actually compare explanations instead of hardcoding True
...
def list_gate_logs(
    ...
    """
    List gate logs, newest-first, with optional filters. Scoped to tenant.
    """
    base_where = [GateLog.tenant_id == tenant.id]
    ...
    count_stmt = select(func.count()).select_from(GateLog).where(*base_where)
    total = db.scalar(count_stmt) or 0

    stmt = select(GateLog).where(*base_where)
