...
Run with:
  cd /home/z/my-project/signalweaver && PYTHONPATH=src python3 -m pytest src/tests/test_analytics.py -v
"""
...
class TestExecutionSummary:
    ...
    def test_summary_counts_match(self, shared_db, client, tenant_a, auth_a):
        ...
        # Create traces with execution logs via the API
        _execute_request(client, "summary-proceed-1", auth_a)  # executed
        _execute_request(client, "summary-proceed-2", auth_a)  # executed

        # Create a blocking scenario
        _create_anchor(shared_db, tid, 3, "Do not help with summary-block-test")
        _execute_request(client, "summary-block-test trigger", auth_a)  # blocked

        resp = client.get("/executions/summary", headers=auth_a)
        assert resp.status_code == 200
        data = resp.json()
        # At minimum: 2 executed + 1 blocked from this test
    ...
    def test_block_rate_calculation(self, shared_db, client, tenant_a, auth_a):
        ...
        # Insert known data directly for precision
        ...
        resp = client.get("/executions/summary", headers=auth_a)
        data = resp.json()
        # Total includes all execution logs for this tenant, so just check
        # the rates are valid floats between 0 and 1
        assert 0.0 <= data["block_rate"] <= 1.0
...
class TestGovernanceInsights:
    ...
    def test_top_conflicted_anchors(self, shared_db, client, tenant_a, auth_a):
        ...
        # Create a blocking anchor and trigger it
        anchor = _create_anchor(
            shared_db, tid, 3, "Do not help with insights-test-violence"
        )

        # Create multiple traces that conflict with this anchor
        for i in range(3):
        ...
        assert resp.status_code == 200
        data = resp.json()
        # At least one anchor should appear
    ...
    def test_top_block_reasons(self, shared_db, client, tenant_a, auth_a):
        ...
        assert resp.status_code == 200
        data = resp.json()
        # Each reason entry has reason + count
...
class TestComplianceExport:
    ...
    def test_export_returns_audit_chain(self, shared_db, client, tenant_a, auth_a):
        ...
        # Execute a request that creates a trace + execution log
        ...
        for t in data["traces"]:
            if t["trace_id"] == trace_id:
                ...
                break
        assert found, "Our trace should be in the export"
    ...
    def test_export_with_blocking(self, shared_db, client, tenant_a, auth_a):
        ...
        resp_status, resp_data = _execute_request(
            client, "compliance-block-test trigger", auth_a
        )
    ...
    def test_export_date_filtering(self, shared_db, client, tenant_a, auth_a):
        ...
        # Filter to last 30 minutes — should NOT include our old trace
        ...
        assert not past_trace_found, "Old trace should not appear in recent filter"
    ...
    def test_export_trace_shape(self, shared_db, client, tenant_a, auth_a):
        ...
        if data["traces"]:
            ...
            for f in required:
                assert f in t, f"Missing field {f} in compliance trace"
...
class TestAnalyticsTenantIsolation:
    ...
    def test_executions_tenant_isolation(self, shared_db, client, tenant_a, tenant_b, auth_a, auth_b):
        ...
        # Check trace IDs don't overlap (each tenant only sees their own)
    ...
    def test_summary_tenant_isolation(self, shared_db, client, tenant_a, tenant_b, auth_a, auth_b):
        ...
        # Both tenants should have positive totals (from other tests too)
        # But the key is they don't see each other's data
    ...
    def test_governance_tenant_isolation(self, shared_db, client, tenant_a, tenant_b, auth_a, auth_b):
        ...
        resp_b = client.get("/governance/insights", headers=auth_b)

        # Tenant A should not see anchor_b's ID in their insights
        anchor_ids_a = {
            e["anchor_id"] for e in resp_a.json()["top_conflicted_anchors"]
        }
...
class TestTimeseriesBucketing:
    ...
    def test_hourly_bucket_boundaries(self):
        ...
        # First bucket starts at the top of the hour containing start (10:00)
        assert buckets[0][0] == datetime(2026, 4, 1, 10, 0)
        assert buckets[-1][1] == datetime(2026, 4, 1, 13, 15)
        # Each bucket should be exactly 1 hour (except last)
    ...
    def test_daily_bucket_boundaries(self):
        ...
        assert len(buckets) == 3  # Apr 1, Apr 2, Apr 3 (partial)
        assert buckets[0][0] == datetime(2026, 4, 1, 0, 0)
        assert buckets[0][1] == datetime(2026, 4, 2, 0, 0)
        assert buckets[2][0] == datetime(2026, 4, 3, 0, 0)
        assert buckets[2][1] == datetime(2026, 4, 3, 12, 0)

    def test_weekly_bucket_boundaries(self):
        """Weekly buckets align to Monday."""
        from app.api.analytics import _generate_buckets
        # April 1, 2026 is Wednesday — Monday of that week is March 30
        start = datetime(2026, 4, 1, 0, 0)
        end = datetime(2026, 4, 15, 0, 0)
        buckets = _generate_buckets(start, end, "week")
        # First bucket starts at the Monday containing start
        assert buckets[0][0] == datetime(2026, 3, 30, 0, 0)
        # Each full bucket is 7 days
...
class TestTimeseriesEndpoint:
    ...
    def test_daily_aggregation_with_data(self, shared_db, client, tenant_a, auth_a):
        ...
        if today_bucket is not None:
            ...
            assert today_bucket["total_requests"] >= 3
            assert 0.0 <= today_bucket["block_rate"] <= 1.0
    ...
    def test_hourly_aggregation(self, shared_db, client, tenant_a, auth_a):
        ...
        # Check each bucket has the right shape
        for b in data["buckets"]:
            assert "start" in b
            assert "end" in b
            assert "total_requests" in b
            assert "executed" in b
            assert "blocked" in b
            assert "failed" in b
            assert "block_rate" in b
            assert "override_rate" in b
            assert "shadow_would_block_rate" in b
    ...
    def test_empty_dataset(self, shared_db, client, tenant_a, auth_a):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert len(data["buckets"]) == 2  # Jan 1 and Jan 2
        for b in data["buckets"]:
            ...
            assert b["override_rate"] == 0.0
            assert b["shadow_would_block_rate"] == 0.0

    def test_invalid_granularity_returns_422(self, client, tenant_a, auth_a):
        """Invalid granularity returns 422."""
        resp = client.get(
            "/executions/timeseries?granularity=monthly", headers=auth_a
        )
        assert resp.status_code == 422
    ...
    def test_start_date_after_end_date_returns_422(self, client, tenant_a, auth_a):
        ...
        resp = client.get(
            "/executions/timeseries?granularity=day"
    ...
    def test_default_date_range_7_days(self, client, tenant_a, auth_a):
        ...
        assert resp.status_code == 200
        data = resp.json()
        # Should produce approximately 7-8 buckets (7 days)
        assert 6 <= len(data["buckets"]) <= 9
    ...
    def test_filter_by_connector(self, shared_db, client, tenant_a, auth_a):
        ...
        # Insert webhook execution
        t2 = _direct_insert_trace(shared_db, tid, "proceed", "ok")
        _direct_insert_exec_log(shared_db, tid, t2.id, "proceed", "executed", connector="webhook")

        resp_mock = client.get(
            "/executions/timeseries?granularity=day&connector=mock",
            headers=auth_a,
        )
        resp_webhook = client.get(
            "/executions/timeseries?granularity=day&connector=webhook",
            headers=auth_a,
        )
        ...
        mock_total = sum(b["total_requests"] for b in resp_mock.json()["buckets"])
        webhook_total = sum(b["total_requests"] for b in resp_webhook.json()["buckets"])
    ...
    def test_filter_by_status(self, shared_db, client, tenant_a, auth_a):
        ...
        resp_exec = client.get(
            "/executions/timeseries?granularity=day&status=executed",
            headers=auth_a,
        )
        resp_blocked = client.get(
            "/executions/timeseries?granularity=day&status=blocked",
            headers=auth_a,
        )

        exec_total = sum(b["total_requests"] for b in resp_exec.json()["buckets"])
        blocked_total = sum(b["total_requests"] for b in resp_blocked.json()["buckets"])

        assert exec_total >= 1
        assert blocked_total >= 1

    def test_rate_calculations_no_division_by_zero(self, shared_db, client, tenant_a, auth_a):
        """Rates are 0.0 when bucket is empty (no division by zero)."""
    ...
    def test_deterministic_output(self, shared_db, client, tenant_a, auth_a):
        """Same input produces same output (uses explicit date range)."""
        start = "2025-06-01T00:00:00"
        end = "2025-06-03T00:00:00"
        url = f"/executions/timeseries?granularity=day&start_date={start}&end_date={end}"
        resp1 = client.get(url, headers=auth_a)
        resp2 = client.get(url, headers=auth_a)
        assert resp1.json() == resp2.json()

    def test_bucket_counts_sum_to_total(self, shared_db, client, tenant_a, auth_a):
        """Sum of per-bucket counts matches the overall summary total."""
        resp_ts = client.get("/executions/timeseries?granularity=day", headers=auth_a)
        resp_summary = client.get("/executions/summary", headers=auth_a)

        ts_data = resp_ts.json()
        summary_data = resp_summary.json()

        ts_total = sum(b["total_requests"] for b in ts_data["buckets"])
        # May not match exactly if default date range differs, but should be <= summary total
        assert ts_total <= summary_data["total_requests"]
    ...
    def test_response_shape(self, client, tenant_a, auth_a):
        ...
        if data["buckets"]:
            b = data["buckets"][0]
            required = [
                "start", "end", "total_requests", "executed", "blocked",
                "failed", "block_rate", "override_rate", "shadow_would_block_rate",
            ]
            for f in required:
                assert f in b, f"Missing field {f} in timeseries bucket"

    def test_no_auth_returns_401(self, client):
        """Timeseries endpoint requires authentication."""
        resp = client.get("/executions/timeseries?granularity=day")
        assert resp.status_code == 401
...
class TestTimeseriesTenantIsolation:
    ...
    def test_tenant_isolation(self, shared_db, client, tenant_a, tenant_b, auth_a, auth_b):
        ...
        _direct_insert_trace(shared_db, tid_a, "proceed", "ok")
        # Need an exec log too
        ...
        # Both should have data, but the totals should be independent
        assert total_a >= 1
        assert total_b >= 1
