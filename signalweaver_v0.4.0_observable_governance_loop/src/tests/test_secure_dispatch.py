...
Covers:
  1. Redaction unit tests
     - authorization/token/api_key/password/signature fields redacted
     - nested sensitive fields redacted
     - list-wrapped sensitive fields redacted
     - non-sensitive fields preserved
     - primitives passed through unchanged
     - empty structures handled

  2. Signing unit tests
     - signature header added when signing_secret present
     - no signature header when absent
     - signature value correct for known payload
     - timestamp header present
     - case-insensitive key matching in redaction of signing_secret

  3. Webhook connector signing integration
     - signed request includes X-SignalWeaver-Signature header
     - signed request includes X-SignalWeaver-Timestamp header
     - unsigned request does not include signing headers
     - signing_secret not in stored result

  4. Storage safety (via /execute/trusted integration)
     - ExecutionLog.response_json does not contain raw secrets
     - webhook target response containing sensitive values gets redacted
     - mock connector echo of sensitive context gets redacted
     - API response does not expose raw secrets

  5. Regression
     - mock connector still works
     - webhook connector still dispatches correctly
     - existing execution semantics unchanged (blocked/executed/failed)

Run with:
  cd /home/z/my-project/signalweaver && PYTHONPATH=src python3 -m pytest src/tests/test_secure_dispatch.py -v
"""
...
from app.models import Base, Tenant, TruthAnchor, ExecutionLog
from app.auth import generate_api_key
from app.connectors.webhook import (
    WebhookConnector, _validate_webhook_config, WebhookValidationError,
)
from app.connectors.redaction import redact_sensitive, REDACT_MARKER, SENSITIVE_PATTERNS
...
class TestRedaction:
    ...
    def test_non_sensitive_fields_preserved(self):
        ...
        assert result == data  # Exact same values
...
class TestSigning:
    ...
    def test_sign_payload_returns_hex_digest(self):
        ...
        assert len(sig) == 64
        # All hex characters
        int(sig, 16)  # Will raise if not hex
    ...
    def test_build_signed_headers_has_timestamp(self):
        ...
        # Timestamp should be a numeric string (unix epoch)
    ...
    def test_build_signed_headers_signature_correct(self):
        ...
        # Recompute from the JSON bytes with compact separators
        import json as _json
        payload_bytes = _json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
        expected = hmac.new(
...
class TestWebhookSigning:
    ...
    def test_non_string_signing_secret_ignored(self):
        ...
        ctx = {
            "url": "https://example.com/webhook",
            "payload": {"data": 1},
            "signing_secret": 12345,  # Not a string
        }
...
class TestStorageSafety:
    ...
    def test_webhook_response_with_sensitive_fields_redacted_in_db(self, shared_db, client, tenant_a, auth_a):
        ...
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        # Target response contains sensitive fields
        mock_resp.json.return_value = {
            "result": "ok",
        ...
        # Check what's stored in ExecutionLog
        ...
        # api_key should be redacted in stored result
        assert stored["response_body"]["api_key"] == REDACT_MARKER
        # user_token is NOT in our sensitive patterns (it's "user_token" not "token")
        # Only exact key matches: "token", "api_key", "apikey", etc.
        # "user_token" does NOT match "token" (it's a different key name)
    ...
    def test_api_response_redacts_sensitive_fields(self, shared_db, client, tenant_a, auth_a):
        ...
        # Token should be redacted in the API response
    ...
    def test_mock_connector_context_redacted_in_db(self, shared_db, client, tenant_a, auth_a):
        ...
        assert elog is not None
        stored = json.loads(elog.response_json)
        # MockConnector echoes context, so api_key and password should be redacted
    ...
    def test_webhook_failure_stored_safely(self, shared_db, client, tenant_a, auth_a):
        ...
        # Error result doesn't contain the payload (only status and error message)
    ...
    def test_webhook_signing_secret_never_persisted(self, shared_db, client, tenant_a, auth_a):
        ...
        # Also verify it's not in the API response
