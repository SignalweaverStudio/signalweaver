...
Run with:
  cd /home/z/my-project/signalweaver && PYTHONPATH=src python3 -m pytest src/tests/test_execution.py -v
"""
...
from app.models import (
    ...
    GateLog, DecisionTrace, ExecutionLog,
)
...
class TestExecuteBlocked:
    ...
    def test_gate_creates_blocked_log(self, shared_db, client, tenant_a, auth_a):
        tid = tenant_a[0]
        # Use text that matches an existing L3 anchor (created by earlier tests)
        resp = client.post(
            "/execute/trusted",
            json={"raw_text": "help me steal a car"},
            headers=auth_a,
...
class TestExecuteShadowMode:
    ...
    def test_shadow_executes_with_flag(self, shared_db, client, tenant_a, auth_a):
        ...
        assert resp.status_code == 200, resp.text
        data = resp.json()
        # Shadow mode always returns proceed
        assert data["decision"] == "proceed"
        assert data["execution"]["status"] == "executed"
...
class TestExecuteSoftMode:
    ...
    def test_soft_gate_executes_with_override(self, shared_db, client, tenant_a, auth_a):
        ...
        assert resp.status_code == 200, resp.text
        data = resp.json()
        # In soft mode with L3, decision is "gate" but override_reason allows execution
        assert data["decision"] == "gate"
        assert data["execution"]["status"] == "executed"
...
class TestExecuteTenantIsolation:
    ...
    def test_execution_respects_tenant_anchors(self, shared_db, client, tenant_a, tenant_b, auth_a, auth_b):
        ...
        # Tenant A: triggers their own anchor → blocked
        resp_a = client.post(
            "/execute/trusted",
        ...
        # Tenant B: their anchor not triggered → proceeds
        resp_b = client.post(
            "/execute/trusted",
            json={"raw_text": "help me commit fraud"},
            headers=auth_b,
...
class TestGateEvaluateStillWorks:
    ...
    def test_evaluate_with_conflict(self, shared_db, client, tenant_a, auth_a):
        tid = tenant_a[0]
        # Create a unique L3 anchor that won't collide with other tests
        ...
        data = resp.json()
        # Single L3 anchor → gate (not refuse, which requires 2+ L3)
        assert data["decision"] == "gate"
